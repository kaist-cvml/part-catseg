...
class Aggregator(nn.Module):
    ...
    def forward(self, 
        ...
        corr = self.correlation(img_feats, text_obj_feats)
        corr_specific_part = self.correlation(img_feats, text_specific_part_feats)
        ...
        corr_specific_part_embed = self.corr_embed(corr_specific_part)
        ...
        if self.text_guidance_projection is not None:
            ...
            projected_text_guidance = self.text_guidance_projection(text_obj_feats)

        # for i, (layer, layer_obj) in enumerate(zip(self.layers, self.layers_object)):
        for i, layer in enumerate(self.layers):
            corr_embed = layer(corr_embed, projected_guidance, projected_text_guidance)
        
        obj_logit = self.conv_decoder(corr_embed, projected_decoder_guidance)
        ...
        for layer in self.layers_specific_part:
            corr_specific_part_embed = layer(
                corr_specific_part_embed, projected_guidance, projected_text_guidance
            )

        specific_part_logit = self.conv_decoder(corr_specific_part_embed, projected_decoder_guidance)

        costs = None
        if return_cost:
            costs = {
                "obj_cost": corr, # B P T H W
            }

        return obj_logit, specific_part_logit, costs
