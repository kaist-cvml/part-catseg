...
class Aggregator(nn.Module):
    ...
    def forward(self, 
        ...
        corr = self.correlation(img_feats, text_part_obj_feats)
        ...
        if self.text_guidance_projection is not None:
            text_part_obj_feats = text_part_obj_feats.mean(dim=-2)
            text_part_obj_feats = text_part_obj_feats / text_part_obj_feats.norm(dim=-1, keepdim=True)
        ...
        part_obj_logit = self.conv_decoder(corr_embed, projected_decoder_guidance)

        b, c, t, h, w = corr_embed.shape
        num_text_classes = text_specific_part_feats.shape[1]

        re_corr_embed = rearrange(corr_embed, 'B C T H W -> B T C (H W)')

        specific_part_embed = torch.zeros((b, num_text_classes - 1, c * 2, h * w), device=re_corr_embed.device)
        specific_part_embed[..., :c, :] += torch.gather(re_corr_embed[:, :num_part_classes], 1, part_index_map.permute(0, 3, 1, 2).repeat(b, 1, *re_corr_embed.shape[2:]))
        specific_part_embed[..., c:, :] += torch.gather(re_corr_embed[:, num_part_classes:], 1, obj_index_map.permute(0, 3, 1, 2).repeat(b, 1, *re_corr_embed.shape[2:]))
        specific_part_embed = torch.cat([specific_part_embed, re_corr_embed[:,-1].repeat(1, 2, 1).unsqueeze(1)], dim=1)
        
        cost_feats = specific_part_embed.reshape(b, num_text_classes, -1, h, w)
        cost_feats = rearrange(cost_feats, 'B T C H W -> (B T) C H W', B=b)
        cost_feats = self.conv_proj_feat(cost_feats)
        cost_feats = rearrange(cost_feats, '(B T) C H W -> B T C H W', B=b)
        
        # Cost Aggregation for Object Specific Parts
        cost_corr = self.correlation_cost(cost_feats, text_specific_part_feats)

        # Specific Part Normalization
        # cost_corr = F.softmax(cost_corr, dim=2)

        cost_corr_embed = self.cost_corr_embed(cost_corr)
        
        if self.text_guidance_projection is not None:
            text_specific_part_feats = text_specific_part_feats.mean(dim=-2)
            text_specific_part_feats = text_specific_part_feats / text_specific_part_feats.norm(dim=-1, keepdim=True)
            projected_text_guidance = self.text_guidance_projection(text_specific_part_feats)

        for layer in self.layers_specific_part:
            cost_corr_embed = layer(
                cost_corr_embed, projected_guidance, projected_text_guidance
            )
            # cost_corr_embed = layer(
            #     cost_corr_embed, text_guidance=projected_text_guidance
            # )

        logit = self.conv_decoder(cost_corr_embed, projected_decoder_guidance)

        if return_cost:
            costs = {
                "part_obj_cost": corr, # B P T H W
                "specific_part_cost": cost_corr, # B P T H W
                "part_obj_cost_emb": corr_embed, # B C T H W
                "specific_part_cost_emb": cost_corr_embed, # B C T H W
            }

            return part_obj_logit, logit, costs

        return part_obj_logit, logit, None
