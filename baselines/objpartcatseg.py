...
# CAT-Seg based on PartCATSeg pipeline for OVOS
class ObjPartCATSeg(nn.Module):
    @configurable
    def __init__(
        self,
        *,
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            obj_outputs, specific_part_outputs, costs = self.sem_seg_head(clip_features, features)
            obj_outputs = F.interpolate(obj_outputs, size=(obj_in_part_targets.shape[-2], obj_in_part_targets.shape[-1]), mode="bilinear", align_corners=False)
            ...
            assert num_classes == num_obj_classes + 1, f"{num_classes} != {num_obj_classes + 1}"
            ...
            obj_targets[..., :-1][obj_mask] = obj_onehot
            ...
            # eps = 1e-10
            # bs, _, _, ch, cw = costs["obj_cost"].shape
            # obj_costs = costs["obj_cost"].permute(0, 2, 1, 3, 4) # B P T H W -> B T P H W (include background)
            ...
            loss = self.lambda_obj * obj_loss + specific_part_loss
            ...
            with torch.no_grad():
                # _, outputs, costs = self.sem_seg_head(clip_features, features)
                outputs, _, costs = self.sem_seg_head(clip_features, features)
                ...
                processed_results = [
                    {'sem_seg': output.detach().cpu(), 'costs': costs}
